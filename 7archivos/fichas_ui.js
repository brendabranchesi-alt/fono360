...
function _htmlLista(p, fichas) {
  var items = fichas.length
    ...
    : '<div class="fc-empty">' +
        ...
        '<p class="fc-empty-title">Sin fichas clínicas</p>' +
        '<p class="fc-empty-sub">Registrá el motivo de consulta,<br>antecedentes y diagnóstico</p>' +
      '</div>';
...
function _htmlCard(f) {
  ...
  return '<div class="fc-card fc-card--anim" data-id="' + f.id + '">' +
...
function _htmlEditor(ficha) {
  ...
  var htmlBase = _FICHA_CAMPOS_BASE.map(function(campo) {
    return _htmlCampo(campo, ficha[campo.id] || '');
  }).join('');
  ...
  return '<div class="fc-wrap">' +
    ...
    '<div class="fc-bloque-base">' +
      '<div class="fc-bloque-label">📋 Diagnóstico clínico</div>' +
...
function _htmlCampo(campo, valor) {
  return '<div class="fc-campo">' +
    '<label class="fc-label" for="fcf_' + campo.id + '">' + campo.label + '</label>' +
    '<textarea' +
      ' class="fc-textarea"' +
      ' id="fcf_' + campo.id + '"' +
      ' data-campo="' + campo.id + '"' +
...
function _fichaInjectStyles() {
  ...
  s.textContent = [
    ...
    // ── Bloque base (diagnóstico clínico) ─────────────────
    '.fc-bloque-base{background:var(--bg2);border:1.5px solid var(--border);' +
      ...
      'box-shadow:var(--sh)}',
    '.fc-bloque-label{font-size:12px;font-weight:700;color:var(--t3);' +
